...
#This might suggest that we go back and check on that one plot point to make sure there was no obvious error
#ultimately, it would be up to the anaylst to include or exclude that data point from the study.
...
#plot we can examine if the observations vary greatly or not. In this case, we see that there is very little variance
#between sex and the seasons. Therefore we may continue on the next step.
...
#We can learn that the two graphs that data may seemed skewed in some cases when grouped all together and not sorted.
#If we had just looked at the summer months, we might assume the data is skewed, but after separating
#the data into their own separated month, we see that the data is actually normal.
...
#plot will give us a general overview of what we should begin to think about as we move along with out dataset
#you can look at the current graph and see there are some relationships between lengths (as expected)
#as head length increases so does weight generally (bigger head more weight) and etc
...
#By creating the two graphs on the left, we can see if there are any obvious patterns of abundance due to time
#We can see on the top there is, but on the bottom there is not (this is due to migration). Therefore we know
#the observations of the response variable are not independent for the top, but are for the bottom.
#Moving to the right two graphs, this is another way to find variable dependence. Any value where the ACF goes above
#the significance threshold, we can assume that the abundance/value there is due to the time period (so abundances
#of the bird are due to the time period)
